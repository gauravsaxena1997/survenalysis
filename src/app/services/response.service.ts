...
export class ResponseService {
  private responseData: Array<any> = [
    {
        "id": "q22ed34",
        "response": {
            "1": [
  ...
  public submitResponse(surveyId: string, surveyResponse: Array<ResponseData>) {
    ...
    const existingResponse = this.getResponseBySurveyId(surveyId);
    if (existingResponse) {
      responseData['response'] = existingResponse;
    }
    ...
    surveyResponse.forEach((quesResponse: ResponseData) => {
      if (quesResponse.type === this.questionTypesObj['Single Punch']) {
        ...
        if (!responseData['response'][quesResponse.questionId]) {
          responseData['response'][quesResponse.questionId] = {};
        }

        if (!responseData['response'][quesResponse.questionId][response]) {
          responseData['response'][quesResponse.questionId][response] = 1;
        } else {
          responseData['response'][quesResponse.questionId][response] += 1;
        }
        ...
        response.forEach((optionText: string) => {
          if (!responseData['response'][quesResponse.questionId][optionText]) {
            responseData['response'][quesResponse.questionId][optionText] = 1;
          } else {
            responseData['response'][quesResponse.questionId][optionText] += 1;
          }
        })
        ...
        responseData['response'][quesResponse.questionId].push(response);
      }
    });
  ...
  public getResponseBySurveyId(surveyId: string): ResponseData {
    const responseData =  this.responseData.find((response: any) => response.id === surveyId);
    return responseData ? responseData.response : null
  }
}
